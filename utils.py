...
def load_dataset(dataset='cifar10', batch_size=128, dataset_root='./data'):
    # Define transforms
    ...
    if dataset == 'cifar10':
        # Use dataset_root instead of hardcoded '/data/cifar_data'
        train_loader = torch.utils.data.DataLoader(
            datasets.CIFAR10(root=dataset_root, train=True, download=True, transform=train_transform_),
            batch_size=batch_size, shuffle=True
        )
        test_loader = torch.utils.data.DataLoader(
            datasets.CIFAR10(root=dataset_root, train=False, download=True, transform=transform_),
        ...
        train_loader = torch.utils.data.DataLoader(
            datasets.CIFAR100(root=dataset_root, train=True, download=True, transform=train_transform_),
            batch_size=batch_size, shuffle=True
        )
        test_loader = torch.utils.data.DataLoader(
            datasets.CIFAR100(root=dataset_root, train=False, download=True, transform=transform_),
            batch_size=batch_size, shuffle=False
        )
        return train_loader, test_loader
